...
export class JournalService {
  ...
  async getEntryById(
    ...
    if (!journal || journal.userId !== userId) {
      // Journal not found or doesn't belong to the user
      // Throw an error or return null, depending on how the controller should handle it.
      // The current controller expects null/undefined and then throws its own error.
      // Alternatively, you could throw AuthorizationError here.
  ...
  async deleteEntry(
    ...
    const deleted = await this.journalRepository.deleteEntryById(entryId, journalId); // Pass journalId for potential check in repo
