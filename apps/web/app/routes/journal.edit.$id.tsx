...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  try {
    ...
    if (_action === "delete") {
      ...
      return redirect("/journal"); // Redirect after successful deletion
    } else {
...
import { lazy, Suspense } from "react"; // Restore lazy

const Editor = lazy(() =>
  import("~/components/ui/CustomTextEditor/RichTextEditor").then((module) => ({
              ...
              <div className="flex flex-col h-full">
                ...
                <div className="flex-1 bg-neutral-50 dark:bg-gray-900 rounded-lg border-2 border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden mb-4 min-h-[200px]">
                  <Suspense
                    fallback={
                      <div
                        className={`p-4 ${navigation.state === "loading" ? "opacity-50" : ""}`}
                      >
                  ...
                  >
                    <Editor
                      onChange={setContent} // Use setContent again
